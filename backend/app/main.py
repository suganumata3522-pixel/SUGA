...
def report_pdf(
    ...
    marks="A,B,C" を渡すと、その符号のみに限定（"表示中の項目のみ出力"=ON 用）。
      その場合、画面で「一致」を表示しているときは「一致」もレポートに含める。
    marks 未指定の場合は ("表示中の項目のみ出力"=OFF) フィルタを無視し、
      その符号の全差分（「一致」を含む）を出力する。
    """
    if not role_paths("drawing") or not role_paths("calc"):
        raise HTTPException(400, "構造図PDFと計算書PDFを両方アップロードしてください")
    drawing_set, drawing_slabs = _parse_drawings()
    ...
    if marks is not None:
        ...
        # OFF: フィルタ無視、全差分（一致を含む）を出力
        summary["出力対象件数"] = len(diffs)
        summary["出力条件"] = "全差分（フィルタ無視・一致含む）"
        include_match = True
