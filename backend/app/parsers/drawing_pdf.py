...
_NON_POSITION_KEYWORDS = (
    "機械式", "継手", "要領", "定着", "増打", "開口", "補強筋",
    # あばら筋幅算定などの詳細表ラベル（"柱主筋径" "d（mm）" 等）。位置行と
    # 同じ y に載って位置ラベルとして拾われると、末尾符号の担当範囲がその
    # 詳細表まで伸び、DP 割り当ても崩れる。
    "径", "mm", "ｍｍ",
)


# 位置ラベルが必ず含む「位置指標」文字。梁リストの配筋位置は
#   全断面 / 端部 / 中央 / 元端 / 先端 / 基端 / 連続端 / 通り芯端(X1端 等) / 中央・X3端
# のいずれかで、必ず「端」「央」または「全断」を含む。これらを含まない語
#（TA13 / →TA10 / STP径ワンサイズダウン / 柱主筋径 / 機械式継手範囲 / d（mm）等の
# あばら筋仕様・詳細表・注記ラベル）は位置ではないと判定できる（ポジティブ判定）。
_POSITION_INDICATORS = ("端", "央", "全断")


def _is_position_label(text: str) -> bool:
    """位置ラベル（配筋位置の列見出し）として妥当かを判定する。

    位置ラベルは 全断面/端部/中央/元端/先端/基端/連続端/通り芯端 等で、必ず
    「端」「央」「全断」のいずれかを含む（ポジティブ判定）。これにより
    あばら筋仕様(TA13/→TA10)・詳細表(柱主筋径/d（mm）)・注記(機械式継手範囲/
    STP径ワンサイズダウン)・開口補強の方向記号(縦/横/斜)・鉄筋値(2-D13) 等の
    位置でない語を確実に除外する。「中央」が「中」「央」に分割される抽出に
    対応するため単独の「中」は許容する。
    """
    if not text or text == "位置":
        return False
    if text == "中":  # 「中央」が分割抽出されたときの前半
        return True
    # 注記引出し語（機械式継手/定着要領/開口補強筋 等）は明示除外
    ...
    if _REBAR_COMBINED_RE.match(text):
        return False
    # 位置指標（端/央/全断）を含まない語は位置ラベルではない
    if not any(ind in text for ind in _POSITION_INDICATORS):
        return False
    return True
