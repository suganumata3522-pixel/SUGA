...
「要目視確認」が増える主な理由:
  ・連梁判定（位置ラベルが通り芯を複数列挙）
  ・構造図 全断面値が計算書の位置別配筋を包絡（複数バリアントあり）
  ・同符号で計算書に複数の検討あり、かつ いずれかの検討内で位置別に
    配筋が異なる（B1A のように端部・中央で配筋が違う場合）
"""
...
EXPECTED: dict[str, dict] = {
    ...
    "project2": {
        ...
        # B2/B3/WB1 は同符号で計算書内に複数の検討があり、いずれかの
        # 検討内で位置別に配筋が異なるため要目視確認。
