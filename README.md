...
実行ファイル `YHG.exe` を 1 つ配るだけ。利用者は Python も Docker も不要。

1. **ビルド担当者が一度だけ** ビルドする（Windows / Python 3.11+ / Node.js 20+ が必要）
   ```
   scripts\build_exe.bat
   ```
   → `backend\dist\YHG.exe`（構造図/計算書チェック本体）と
   　 `backend\dist\YHG-Sleeve.exe`（梁スリーブ貫通補強チェック、実装準備中）が生成される
2. それらの exe を各利用者へ配布（社内共有フォルダ等）
3. 利用者は exe をダブルクリック → 自動でブラウザが開く
   - データ（案件・PDF）は exe と同じ場所の `YHG-data/` / `YHG-Sleeve-data/` に保存される
   - 終了するときはコンソールウィンドウを閉じる

> **YHG-Sleeve.exe について** — 梁スリーブ貫通補強の整合チェック機能は現在実装準備中。
> サンプル計算書PDF が手元に揃い次第、パーサと照合ロジックを実装し有効化する。
> 起動すると現状は「実装準備中」案内画面が表示される（YHG.exe とは別ポート 8001 で動作）。
